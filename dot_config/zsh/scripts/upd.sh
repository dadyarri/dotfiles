...
{
	local first_option all_short_options='srfvh'
	first_option="${1:0:1}"
...
print_help()
{
	...
	printf 'Usage: %s [-s|--(no-)system] [-r|--(no-)autoremove] [-f|--(no-)flatpak] [-v|--(no-)verbose] [-h|--help]\n' "$0"
...
if [[ $_arg_verbose == 'on' ]]; then
  printf "'%s' is %s\\n" 'system' "$_arg_system"
  printf "'%s' is %s\\n" 'autoremove' "$_arg_autoremove"
  printf "'%s' is %s\\n" 'flatpak' "$_arg_flatpak"
fi

# ------------------

ansi --blue --newline "Updating system started..."
...
if [[ $_arg_system == 'on' ]]; then
  ansi --blue --newline "Updating packages..."
...
if [[ $_arg_autoremove == 'on' ]]; then
  ansi --blue --newline "Removing orphans..."
...
if [[ $_arg_flatpak == 'on' ]]; then
  ansi --blue --newline "Updating flatpak apps..."
  flatpak update -y
